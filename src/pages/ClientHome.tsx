...
function usePushSubscription({ clientId, clientName, artistProfileId, lang, onInstallNeeded }: { clientId: string; clientName: string; artistProfileId: string; lang: 'en' | 'he'; onInstallNeeded?: (code: string) => void }) {
  ...
  useEffect(() => {
    ...
    const lsSubscribed = localStorage.getItem(lsKey) === '1';
    const permissionGranted = 'Notification' in window && Notification.permission === 'granted';

    // Fast path: LS flag + permission still granted → already subscribed, no network call needed.
    // Doing a browser pushManager check on every load is unreliable (Safari returns null on refresh
    // even with a valid subscription) and causes silent re-subscribe loops that clear the flag.
    if (lsSubscribed && permissionGranted) {
      setStatus('subscribed');
      return;
    }

    // No LS flag — check DB once to recover any subscription the user set up on another device
    (async () => {
      try {
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Install-related failures: surface the visual install guide — UNLESS the user already
        // dismissed it with "Maybe later" (glow-pwa-dismissed). Then respect their choice and
        // show the guidance as a toast instead of re-popping the modal on every Enable click.
        const installCodes = ['ios-needs-install', 'in-app-browser', 'ios-push-unavailable'];
        let dismissedInstall = false;
        try { dismissedInstall = localStorage.getItem('glow-pwa-dismissed') === '1'; } catch {}
        if (result.code && installCodes.includes(result.code) && !dismissedInstall) {
