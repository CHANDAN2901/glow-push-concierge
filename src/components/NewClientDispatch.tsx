...
const NewClientDispatch = ({
  ...
  const handleSendWhatsApp = async () => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: 'הצהרת בריאות לטיפול', text: msg });
        markDispatched(link);
        return;
      } catch (err: any) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex flex-col bg-background">
      ...
      <div className="flex-shrink-0 px-4 py-3.5 flex items-center gap-3" style={{ borderBottom: `1px solid ${GOLD}30` }}>
        <button onClick={handleClose} className="p-2 rounded-full transition-colors" style={{ color: GOLD_DARK }}>
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input value={name} onChange={e => setName(e.target.value)}
                ...
                className={inputStyle} style={{ border: GOLD_BORDER }} />
            </div>
            <div>
              <Label className="text-xs font-bold mb-1.5 block" style={{ color: GOLD_DARK }}>
                {lang === 'en' ? 'Phone Number' : 'מספר טלפון'}
              </Label>
              <Input value={phone} onChange={e => { setPhone(e.target.value); setDuplicateAck(false); }}
                placeholder="050-1234567" type="tel" className={inputStyle}
                style={{ border: GOLD_BORDER }} dir="ltr" />
            </div>
            <div>
              <Label className="text-xs font-bold mb-1.5 block" style={{ color: GOLD_DARK }}>
              ...
              <Select value={treatment} onValueChange={setTreatment}>
                <SelectTrigger className="h-12 rounded-full bg-white text-base px-5" style={{ border: GOLD_BORDER }}>
                  <SelectValue placeholder={lang === 'en' ? 'Select treatment...' : 'בחרי סוג טיפול...'} />
                </SelectTrigger>
                <SelectContent className="z-[70]">
                  {treatmentOptions.map(opt => (
                    <SelectItem key={opt.value} value={opt.value}>
                      {opt.value === 'eyebrows' ? '✍️ ' : '👄 '}{lang === 'en' ? opt.en : opt.he}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
      ...
      <div className="flex-shrink-0 border-t border-border/60 bg-background px-5 py-4 space-y-2.5 max-w-md mx-auto w-full"
        style={{ boxShadow: '0 -4px 16px rgba(0,0,0,0.05)' }}>
        {/* Primary: Save & Send Link */}
        <button onClick={handleSendWhatsApp} disabled={!isValid || (isDuplicate && !duplicateAck)}
          ...
          style={{ background: GOLD_GRADIENT, color: GOLD_TEXT, boxShadow: '0 4px 18px rgba(212, 175, 55, 0.35)' }}>
          <Share2 className="w-5 h-5" />
          {lang === 'en' ? 'Save & Send Link' : 'שמירה ושליחת לינק'}
        </button>

        {/* Secondary row: Save Only | Fill Here | Copy */}
        <div className="flex gap-2">
          <button onClick={handleSaveOnly} disabled={!isValid}
            className="flex-1 py-2.5 rounded-full font-bold text-xs flex items-center justify-center gap-1.5 transition-all active:scale-95 disabled:opacity-40 disabled:pointer-events-none"
            style={{ border: GOLD_BORDER, color: GOLD_TEXT, background: 'transparent' }}>
            <CheckCircle className="w-4 h-4" />
            {lang === 'en' ? 'Save Only' : 'שמירה בלבד'}
          </button>
          <button onClick={handleFillHere} disabled={!isValid}
            className="flex-1 py-2.5 rounded-full font-bold text-xs flex items-center justify-center gap-1.5 transition-all active:scale-95 disabled:opacity-40 disabled:pointer-events-none"
            style={{ border: GOLD_BORDER, color: GOLD_TEXT, background: 'transparent' }}>
            <Smartphone className="w-4 h-4" />
            {lang === 'en' ? 'Fill Here' : 'מילוי במקום'}
          </button>
          <button onClick={handleCopyLink} disabled={!isValid || (isDuplicate && !duplicateAck)}
            className="flex-1 py-2.5 rounded-full font-bold text-xs flex items-center justify-center gap-1.5 transition-all active:scale-95 disabled:opacity-40 disabled:pointer-events-none"
            style={{ border: GOLD_BORDER, color: GOLD_TEXT, background: 'transparent' }}>
            {copied ? <CheckCircle className="w-4 h-4" style={{ color: '#22c55e' }} /> : <Copy className="w-4 h-4" />}
            {copied ? (lang === 'en' ? 'Copied!' : 'הועתק!') : (lang === 'en' ? 'Copy' : 'העתקה')}
          </button>
        </div>
      </div>
    </div>
