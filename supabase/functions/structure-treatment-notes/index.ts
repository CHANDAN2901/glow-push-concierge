...
serve(async (req: Request) => {
  ...
  if (!auth) {
    return new Response(
      JSON.stringify({ error: "Authentication required. Please log in to use AI treatment notes." }),
      { status: 401, headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  }

  console.log("[structure-treatment-notes] Authenticated user:", auth.userId);
